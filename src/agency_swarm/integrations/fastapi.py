...
def run_fastapi(
    ...
    if agencies:
        for agency_name, agency_factory in agencies.items():
            ...
            endpoints_added = app.add_agency(agency_name, agency_factory, handlers)
            endpoints.extend(endpoints_added)

    if tools:
        for tool in tools:
            tool_handler = make_tool_endpoint(tool, verify_token)
            endpoint = app.add_tool(tool, tool_handler)
